...
public class MemoryCache<Key, Type> {
    ...
    public Type get(Key key, Type fallback) {
        return this.get(key, () -> fallback);
    }

    public Type get(Key key, Supplier<Type> fallback) {
        synchronized (this) {
    ...
    * */
    public CompletableFuture<Type> getAsync(Key key, Supplier<CompletableFuture<Type>> fallback) {
        synchronized (this) {
            if(cacheMap.containsKey(key)) {
                // Cache hit
                Type result = cacheMap.get(key).getValue();
    ...
    private Type cache(Key key, Type value) {
        synchronized (this) {
